...
if (isset($_POST['add'])) {
	if (!empty($_POST['subject'])) {		
		...
		if ($upload_file && empty($files_upload)) {
			...
			header('Location: ' . $config->get('address') . '/tickets/index/');
			exit;
		}
	}
	else {
	...
	<form method="post" enctype="multipart/form-data" action="<?php echo safe_output($_SERVER['REQUEST_URI']); ?>">
		...
		<div class="col-md-9">
							...
							<div class="col-lg-12">	
								...
								<?php if ($auth->can('manage_tickets') || $auth->can('tickets_view_assigned_department')) { ?>
										...
										<p><?php echo safe_output($language->get('Carbon Copy')); ?>
											...
											<br />
											<input type="text" name="cc" class="form-control" placeholder="مثال: user@nrskh.ir" value="<?php if (isset($_POST['cc'])) echo safe_output($_POST['cc']); ?>" size="50" />
										</p>
			...
			<div class="col-md-12">
				...
				<div class="pull-right">
					<p><button type="submit" name="add" class="btn btn-primary"><?php echo safe_output($language->get('Add Ticket')); ?></button>
					<a href="<?php echo $config->get('address'); ?>/tickets/index/<?php  ?>" class="btn btn-default"><?php echo safe_output($language->get('Cancel')); ?></a>
					</p>
				</div>
			</div>
