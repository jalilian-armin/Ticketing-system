 ...
 * http://jqueryui.com
 * Includes: jquery.ui.core.js, jquery.ui.datetimepicker.js
    ...
    $.extend(datetimepicker.prototype, {
	   ...
	   @return Date - the current date */
        ...
        _doKeyDown: function(event) {
            ...
            var handled = true;
            var isRTL = inst.dpDiv.is('.ui-datetimepicker-rtl');
            inst._keyEvent = true;
            if ($.datetimepicker._datetimepickerShowing)
                switch (event.keyCode) {
                    ...
                    case 13:
                        var sel = $('td.' + $.datetimepicker._dayOverClass + ':not(.' +
        ...
        _doKeyUp: function(event) {
            ...
            if (inst.input.val() != inst.lastVal) {
                try {
                    var date = $.datetimepicker.parseDate($.datetimepicker._get(inst, 'dateFormat'),
	   ...
	   @param  input  element - the input field attached to the date picker or
        ...
        _showdatetimepicker: function(input) {
            ...
            if (!inst.inline) {
                ...
                var postProcess = function() {
                    var cover = inst.dpDiv.find('iframe.ui-datetimepicker-cover'); // IE6- only
                    if( !! cover.length ){
        ...
        _updatedatetimepicker: function(inst) {
            ...
            if (inst == $.datetimepicker._curInst && $.datetimepicker._datetimepickerShowing && inst.input &&
	 ...
	 */
        ...
        _generateHTML: function(inst) {
            ...
            var prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
                '<a class="ui-datetimepicker-prev btn btn-link" data-handler="prev" data-event="click"' +
                ' title="' + prevText + '">' + (isRTL ? '<i class="icon-circle-arrow-right"></i>' : '<i class="icon-circle-arrow-left"></i>') + '</a>' :
                (hideIfNoPrevNext ? '' : '<a class="ui-datetimepicker-prev btn btn-link ui-state-disabled" title="'+ prevText +'">' + (isRTL ? '<i class="icon-circle-arrow-right"></i>' : '<i class="icon-circle-arrow-left"></i>') + '</a>'));
            var nextText = this._get(inst, 'nextText');
            nextText = (!navigationAsDateFormat ? nextText : this.formatDate(nextText,
                this._daylightSavingAdjust(new this.CDate(drawYear, drawMonth + stepMonths, 1)),//[CC]
                this._getFormatConfig(inst)));
            var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
                '<a class="ui-datetimepicker-next btn btn-link" data-handler="next" data-event="click"' +
                ' title="' + nextText + '">' + (isRTL ? '<i class="icon-circle-arrow-left"></i>' : '<i class="icon-circle-arrow-right"></i>') + '</a>' :
                (hideIfNoPrevNext ? '' : '<a class="ui-datetimepicker-next btn btn-link ui-state-disabled" title="'+ nextText + '">' + (isRTL ? '<i class="icon-circle-arrow-left"></i>' : '<i class="icon-circle-arrow-right"></i>') + '</a>'));
            var currentText = this._get(inst, 'currentText');
            var gotoDate = (this._get(inst, 'gotoCurrent') && inst.currentDay ? currentDate : today);
            currentText = (!navigationAsDateFormat ? currentText :
                this.formatDate(currentText, gotoDate, this._getFormatConfig(inst)));
            var controls = (!inst.inline ? '<button type="button" class="ui-datetimepicker-close btn" data-handler="hide" data-event="click">' +
                this._get(inst, 'closeText') + '</button>' : '');
            var buttonPanel = (showButtonPanel) ? '<div class="ui-datetimepicker-buttonpane ui-helper-clearfix">' + (isRTL ? controls : '') +
            (this._isInRange(inst, gotoDate) ? '<button type="button" class="ui-datetimepicker-current btn" data-handler="today" data-event="click"' +
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    if (isMultiMonth) {
                        ...
                        if (numMonths[1] > 1)
                            switch (col) {
                                case 0:
                                    calender += ' ui-datetimepicker-group-first';
                                    cornerClass = ' ui-corner-' + (isRTL ? 'right' : 'left');
                                    break;
                                case numMonths[1]-1:
                                    calender += ' ui-datetimepicker-group-last';
                                    cornerClass = ' ui-corner-' + (isRTL ? 'left' : 'right');
                                    break;
                                default:
                                    calender += ' ui-datetimepicker-group-middle';
                                    cornerClass = '';
                                    break;
                            }
                        calender += '">';
                    ...
                    '<tr>';
                    var thead = (showWeek ? '<th class="ui-datetimepicker-week-col">' + this._get(inst, 'weekHeader') + '</th>' : '');
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        calender += '<tr>';
                        var tbody = (!showWeek ? '' : '<td class="ui-datetimepicker-week-col">' +
            ...
            html += buttonPanel + ($.ui.ie6 && !inst.inline ?
                '<iframe src="javascript:false;" class="ui-datetimepicker-cover" frameborder="0"></iframe>' : '');
        ...
        _generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
            ...
            var html = '<div class="ui-datetimepicker-title">';
            var monthHtml = '';
            // month selection
 ...
 */
    function bindHover(dpDiv) {
        ...
        return dpDiv.delegate(selector, 'mouseout', function() {
            ...
            if (this.className.indexOf('ui-datetimepicker-prev') != -1) $(this).removeClass('ui-datetimepicker-prev-hover');
            if (this.className.indexOf('ui-datetimepicker-next') != -1) $(this).removeClass('ui-datetimepicker-next-hover');
        })
        .delegate(selector, 'mouseover', function(){
            if (!$.datetimepicker._isDisableddatetimepicker( instActive.inline ? dpDiv.parent()[0] : instActive.input[0])) {
                ...
                if (this.className.indexOf('ui-datetimepicker-prev') != -1) $(this).addClass('ui-datetimepicker-prev-hover');
                if (this.className.indexOf('ui-datetimepicker-next') != -1) $(this).addClass('ui-datetimepicker-next-hover');
            }
        });
    }
   ...
   @return  jQuery object */
    $.fn.datetimepicker = function(options){
        ...
        if (!$.datetimepicker.initialized) {
            ...
            find(document.body).append($.datetimepicker.dpDiv);
